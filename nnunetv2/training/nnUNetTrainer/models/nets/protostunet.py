...
from nnunetv2.training.nnUNetTrainer.models.modules.contrast import momentum_update, l2_normalize, ProjectionHead, ProjectionHead_3D
from nnunetv2.training.nnUNetTrainer.models.modules.sinkhorn import distributed_sinkhorn
from timm.models.layers import trunc_normal_
from einops import rearrange, repeat

class ProtoStuNet(nn.Module):
    """
    带有prototype的模型结构
    ...
    def __init__(self, plans_manager,
        ...
        super(ProtoStuNet, self).__init__()
    ...
    def forward(self, x_, gt_semantic_seg=None, pretrain_prototype=False):
        ...
        mode_interpolate = "bilinear" if len(x[0].size()) == 4 else "trilinear"
        ...
        feat5 = F.interpolate(x[4], size=(d, h, w), mode=mode_interpolate, align_corners=True)
        # feat6 = F.interpolate(x[5], size=(d, h, w), mode=mode_interpolate, align_corners=True)

        feats = torch.cat([feat1, feat3, feat5], 1)
        # feats = x[3]
        if len(x[0].size()) == 4 :
        ...
        if len(x[0].size()) == 4 :
            ...
            out_seg = rearrange(out_seg, "(b h w d) k -> b k d h w", b=feats.shape[0], h=feats.shape[3], d=feats.shape[2])
